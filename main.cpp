...
void problem_10(double f) {
    ...
    for (int n = 2; n<26; n++){
        ...
        double fraction = simulator(100, 100, 100, "w", false, false, true, true, f, (n/10.0)*pow(10,6), false);
        ofile << std::setw(width) << std::setprecision(prec) << std::scientific << fraction;
        ofile << std::endl;
    }
    ofile.close();
...
double simulator(int iterations, int duration, int particles, std::string outputs, bool interactions, bool euler_cromer, bool pertrubation, bool randomseed, double f, double w_v, bool out) {
    ...
    for (int i = 1; i < n+1; i++) {
        ...
        if (pertrubation) {
            penning_trap.V0_ = penning_trap.V0_ * (1 + f * cos (w_v * h*i));
        }
      ...
      ofile.close();
      double tes = penning_trap.particles_inside()/penning_trap.particles_.size();
